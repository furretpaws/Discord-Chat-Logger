// Generated by Haxe 4.3.1
#ifndef INCLUDED_tink_core_OwnedDisposable
#define INCLUDED_tink_core_OwnedDisposable

#ifndef HXCPP_H
#include <hxcpp.h>
#endif

#ifndef INCLUDED_tink_core_Disposable
#include <tink/core/Disposable.h>
#endif
HX_DECLARE_CLASS2(tink,core,Disposable)
HX_DECLARE_CLASS2(tink,core,OwnedDisposable)

namespace tink{
namespace core{


class HXCPP_CLASS_ATTRIBUTES OwnedDisposable_obj {
	public:
		typedef  ::tink::core::Disposable_obj super;
		HX_DO_INTERFACE_RTTI;

		bool (::hx::Object :: *_hx_get_disposed)(); 
		static inline bool get_disposed( ::Dynamic _hx_) {
			return (_hx_.mPtr->*( ::hx::interface_cast< ::tink::core::OwnedDisposable_obj *>(_hx_.mPtr->_hx_getInterface(0xa1838594)))->_hx_get_disposed)();
		}
		void (::hx::Object :: *_hx_ondispose)( ::Dynamic d); 
		static inline void ondispose( ::Dynamic _hx_, ::Dynamic d) {
			(_hx_.mPtr->*( ::hx::interface_cast< ::tink::core::OwnedDisposable_obj *>(_hx_.mPtr->_hx_getInterface(0xa1838594)))->_hx_ondispose)(d);
		}
		void (::hx::Object :: *_hx_dispose)(); 
		static inline void dispose( ::Dynamic _hx_) {
			(_hx_.mPtr->*( ::hx::interface_cast< ::tink::core::OwnedDisposable_obj *>(_hx_.mPtr->_hx_getInterface(0xa1838594)))->_hx_dispose)();
		}
};

} // end namespace tink
} // end namespace core

#endif /* INCLUDED_tink_core_OwnedDisposable */ 
